 ...
 *      and Invocations.
 * @author @nftchance
 * @author @nftchance/emporium-types (2023-10-25)
...
abstract contract Types is ITypes {
	 ...
	 * }>>
	 */
	bytes32 constant SIGNED_DELEGATION_TYPEHASH =
		keccak256(
	 ...
	 * }>>
	 */
	bytes32 constant INVOCATION_TYPEHASH =
		keccak256(
	 ...
	 * }>>
	 */
	bytes32 constant INVOCATIONS_TYPEHASH =
		keccak256(
	 ...
	 * }>>
	 */
	bytes32 constant SIGNED_INVOCATIONS_TYPEHASH =
		keccak256(
	...
	function getArrayPacketHash(
		...
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getPacketHash($input[i]));
	...
	function getPacketHash(
		...
		$packetHash = keccak256(
			abi.encode(
				...
				getPacketHash($input.invocations),
				keccak256($input.signature)
			)
		);
	}
	...
	function getDigest(
		...
		$digest = keccak256(
			abi.encodePacked('\x19\x01', domainHash, getPacketHash($input))
		);
	}
	...
	function getSigner(
		...
		$signer = getDigest($input.delegation).recover($input.signature);
	}
	...
	function getSigner(
		...
		$signer = getDigest($input.invocations).recover($input.signature);
	}
}
