...
describe('Plug Factory', function () {
	...
	it('pass: deploy()', async function () {
		const { contract, implementation, notOwner } = await loadFixture(deploy)

		const address = getAddress(notOwner.account.address)

		const salt = hexToBytes(address, { size: 32 })

		await contract.write.deploy([
