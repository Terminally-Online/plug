...
contract WindowFuse is Fuse {
    ...
    function toWindows(
        uint256 $schedule,
        uint32 $n
    ) external view returns (WindowFuseHelpers.Window[] memory $windows, uint32 $cursor) {
        /// @dev Load the stack.
    ...
    function _isWithinWindow(
        ...
        if (currentTime < $startTime) revert WindowFuseHelpers.WindowLackingStartTime();
