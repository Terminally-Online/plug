 ...
 *      and Intents.
 * @author @nftchance
 * @author @nftchance/plug-types (2023-11-03)
...
abstract contract Types is ITypes {
	 ...
	 *      }>>
	 */
	bytes32 constant CAVEAT_TYPEHASH =
	 ...
	 *      }>>
	 */
	bytes32 constant PERMISSION_TYPEHASH =
		keccak256(
	...
	bytes32 constant TRANSACTION_TYPEHASH =
		keccak256('Transaction(address to,uint256 gasLimit,bytes data)');
	 ...
	 *      }>>
	 */
	bytes32 constant SIGNED_PERMISSION_TYPEHASH =
		keccak256(
	 ...
	 *      }>>
	 */
	bytes32 constant REPLAY_PROTECTION_TYPEHASH =
		keccak256('ReplayProtection(uint256 nonce,uint256 queue)');
	 ...
	 *      }>>
	 */
	bytes32 constant INTENTS_TYPEHASH =
		keccak256(
	 ...
	 *      }>>
	 */
	bytes32 constant SIGNED_INTENTS_TYPEHASH =
		keccak256(
	...
	function getCaveatArrayHash(
		...
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getCaveatHash($input[i]));
	...
	function getSignedPermissionArrayHash(
		...
		/// @dev Encode each item in the array.
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getSignedPermissionHash($input[i]));
	...
	function getReplayProtectionHash(
		...
		$hash = keccak256(
			abi.encode(REPLAY_PROTECTION_TYPEHASH, $input.nonce, $input.queue)
		);
	}
	...
	function getIntentArrayHash(
		...
		/// @dev Encode each item in the array.
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getIntentHash($input[i]));
	...
	function getPermissionDigest(
		...
		$digest = keccak256(
			abi.encodePacked('\x19\x01', domainHash, getPermissionHash($input))
		);
	}
	...
	function getIntentsDigest(
		...
		$digest = keccak256(
			abi.encodePacked('\x19\x01', domainHash, getIntentsHash($input))
		);
	}
	...
	function getSignedIntentsSigner(
		...
		$signer = getIntentsDigest($input.intents).recover($input.signature);
	}
}
