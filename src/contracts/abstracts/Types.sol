...
abstract contract Types is ITypes {
	...
	constructor(string memory $name, string memory $version) {
		...
		domainHash = getEIP712DomainPacketHash(
			EIP712Domain({
				name: $name,
				version: $version,
	...
	function getCaveatArrayPacketHash(
		...
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getCaveatPacketHash($input[i]));
	...
	function getInvocationArrayPacketHash(
		...
		for (i; i < length; ) {
			encoded = bytes.concat(encoded, getInvocationPacketHash($input[i]));
