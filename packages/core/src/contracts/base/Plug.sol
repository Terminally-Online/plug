...
contract Plug is PlugInterface {
	...
	function plug(
		...
		(PlugTypesLib.Result memory results, bytes32 livePlugsHash) = _plug(
			$livePlugs,
			msg.sender
		);
	...
	function _hash(
		...
		PlugTypesLib.LivePlugs calldata $livePlugs
	) internal view returns (bytes32 $livePlugsHash) {
		...
		returns (PlugTypesLib.Result memory $results, bytes32 $livePlugsHash)
	{
		...
		try socket.plug($livePlugs, $sender) returns (
			...
			(uint8 index, string memory reason) = abi.decode(
				slicedData,
				(uint8, string)
			);
