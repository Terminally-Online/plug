    ...
    ReentrancyGuard
{
    ...
    modifier enforceSignature(PlugTypesLib.LivePlugs calldata $input) {
        if (_enforceSignature($input) == false) {
            revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreSignatureInvalid);
        }
        _;
    }
    ...
    modifier enforceSender() {
        if (_enforceSender(msg.sender) == false) {
            revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreSenderInvalid);
        }
        _;
    }
    ...
    modifier enforceSolver(bytes calldata $proof, address $solver) {
        if ($proof.length != 0) {
            if ($proof.length < 0x40) {
                revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreSolverMalformed);
            }
            if (uint256(LibBytes.loadCalldata($proof, 0x00)) < block.timestamp) {
                revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreSolverExpired);
            }
            if (address(uint160(uint256(LibBytes.loadCalldata($proof, 0x20)))) != $solver) {
                revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreSolverInvalid);
            }
        }
        _;
    }
        ...
        returns (bool $allowed)
    {
        ...
        if (nonces.get(nonce) == true) {
            revert PlugLib.PlugFailed(type(uint8).max, PlugLib.PlugCoreNonceInvalid);
        }
        nonces.set(nonce);
     ...
     */
    function _call(bytes memory $plug) internal returns (bool $success, bytes memory $result) {
        (uint8 selector, address to, uint256 value, bytes memory data) = $plug.decode();
        ...
        enforceSolver($plugs.solver, $solver)
    {
        ...
        for (uint256 i; i < $plugs.plugs.length; i++) {
            ...
            for (uint256 ii; ii < $plugs.plugs[i].updates.length; ii++) {
                (state[i],) = state[$plugs.plugs[i].updates[ii].slice.index].transform(
                    i, $plugs.plugs[i].updates[ii], state[i]
                );
            }
