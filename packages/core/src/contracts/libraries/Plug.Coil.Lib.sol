...
library PlugCoilLib {
     ...
     */
    function decode(bytes memory $coil)
        internal
        pure
        returns (
            uint8 $selector,
    ...
    {
        ($selector, $to, $value, $data) =
            abi.decode($coil, (uint8, address, uint256, bytes));
    }
     ...
     * @return $selector The 8-bit selector identifying the operation type
     */
    function selector(bytes memory $coil)
        internal
        pure
     ...
     * @return $data The calldata to execute with the operation
     */
    function data(bytes memory $coil)
        internal
        pure
    ...
    function transform(
        ...
        internal
        pure
        returns (bytes memory $charge, uint256 $area)
    {
        ...
        returns (bytes memory $charge, uint256 $area)
    {
        ...
        $charge = $coil.slice(
            $update.slice.start, $update.slice.start + $update.slice.length
        );
        ...
        returns (bytes memory $charge, uint256 $area)
    {
        ...
        if ($update.slice.typeId == TYPE_ARRAY) {
            ...
            if (arrayLength * WORD > dataLength) {
                revert PlugLib.PlugFailed(
                    $i, PlugLib.PlugCoreArrayLengthInvalid
                );
            }
        ...
        $charge = LibBytes.slice(
            $coil, dataOffset + WORD, dataOffset + WORD + dataLength
        );
