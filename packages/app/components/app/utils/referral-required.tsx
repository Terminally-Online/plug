...
import { cn, greenGradientStyle, useConnect, useMediaQuery } from "@/lib"
	...
	return (
		<div className="flex w-full flex-col items-start justify-between bg-white p-2">
			{!md && (
				<button
					onClick={() => signOut()}
					className="fixed right-4 top-4 p-2 text-sm font-bold opacity-40 hover:opacity-100"
				>
					Logout
				</button>
			)}
					...
					<div className="w-full space-y-6">
						...
						<div className="flex flex-col gap-2">
							...
							<div className="flex flex-col gap-2 sm:flex-row">
								<Button
									variant="secondary"
									className="w-full py-4 sm:w-max"
									onClick={handleRequestAccess}
